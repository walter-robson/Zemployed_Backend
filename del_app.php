...
$app = $_GET['application_id'];

$job_query = "select job_id from application where id = $app limit 1";

$job_query_results = $link->query($job_query) or die ('Job Query Failed');
while($job_tuple = $job_query_results->fetch_array(MYSQLI_ASSOC)){
	foreach($job_tuple as $col_val){
		$job=$col_val;
	}
} 


$del_query = "DELETE FROM  application WHERE id = $app";
$del_results = $link->query($del_query) or die ('Query Failed');
