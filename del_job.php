...
$del_query = "DELETE FROM  job WHERE id = $job";
$del_results = $link->query($del_query) or die ('Query Failed');

$query = "select id, title, date, recruiter_id from job";
$results = $link->query($query) or die ('Query Failed');

echo '<table class="styled-table hover-table"\n';
echo "\t<tr><td>ID</td><td>Title</td><td>Start Date</td><td>Recruiter ID</td></tr>\n";
